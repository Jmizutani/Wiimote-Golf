...
public class SWController : MonoBehaviour {
    ...
    void Start()
    {
        ...
        golfer = GameObject.Find("golfer_SW");
    ...
    void Update()
    {
        ...
        if (stateInfo.normalizedTime > 0.9f)
        {
            if (flag == false)
            {
                ball.GetComponent<Rigidbody>().AddForce(-2.2f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 1.8f * speed, -2.2f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
    ...
    void OnTriggerEnter(Collider hit)
    {
        if (hit.CompareTag("ball"))
        {
            if (flag == false)
            {
                hit.gameObject.GetComponent<Rigidbody>().AddForce(-2.2f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 1.8f * speed, -2.2f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
