...
public class I9Controller : MonoBehaviour
{
    private float theta;
    ...
    void Start()
    {
        ...
        golfer = GameObject.Find("golfer_I9");
    ...
    void Update()
    {
        ...
        temp_speed = (cliant.GetComponent<WiimoteInfoDisplayBase>().balanceBoardData.yaw - offset_yaw) / 740f;
        if (speed < temp_speed)
        {
            speed = temp_speed;
        }
        ...
        if (stateInfo.normalizedTime > 0.9f)
        {
            if (flag == false)
            {
                ball.GetComponent<Rigidbody>().AddForce(-2.8f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2f * speed, -2.8f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
    ...
    void OnTriggerEnter(Collider hit)
    {
        if (hit.CompareTag("ball"))
        {
            if (flag == false)
            {
                hit.gameObject.GetComponent<Rigidbody>().AddForce(-2.8f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2f * speed, -2.8f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
