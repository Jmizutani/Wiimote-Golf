...
public class I8Controller : MonoBehaviour {
    private float theta;
    ...
    void Start()
    {
        ...
        golfer = GameObject.Find("golfer_I8");
    ...
    void Update()
    {
        ...
        if (stateInfo.normalizedTime > 0.9f)
        {
            if (flag == false)
            {
                ball.GetComponent<Rigidbody>().AddForce(-2.9f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2f * speed, -2.9f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
    ...
    void OnTriggerEnter(Collider hit)
    {
        if (hit.CompareTag("ball"))
        {
            if (flag == false)
            {
                hit.gameObject.GetComponent<Rigidbody>().AddForce(-2.9f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2f * speed, -2.9f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
