...
public class PutterController : MonoBehaviour {
    ...
    void Start()
    {
        ...
        golfer = GameObject.Find("golfer_Putter");
    ...
    void Update()
    {
        ...
        temp_speed = (cliant.GetComponent<WiimoteInfoDisplayBase>().balanceBoardData.yaw - offset_yaw) / 800f;
        if (speed < temp_speed)
        {
            speed = temp_speed;
        }
        ...
        if (stateInfo.normalizedTime > 0.9f)
        {
            if (flag == false)
            {
                ball.GetComponent<Rigidbody>().AddForce(-4.2f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 0f, -4.2f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
                flag = true;
            }
        }
    }
    ...
    void OnTriggerEnter(Collider hit)
    {
        if (hit.CompareTag("ball"))
        {
            if (flag == false)
            {
                hit.gameObject.GetComponent<Rigidbody>().AddForce(-4.2f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 0f, -4.2f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
                flag = true;
            }
        }
    }
