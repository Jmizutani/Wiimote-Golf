...
public class I7Controller : MonoBehaviour {
    private float theta;
    ...
    void Start()
    {
        ...
        golfer = GameObject.Find("golfer_I7");
    ...
    void Update()
    {
        ...
        temp_speed = (cliant.GetComponent<WiimoteInfoDisplayBase>().balanceBoardData.yaw - offset_yaw) / 730f;
        if (speed < temp_speed)
        {
            speed = temp_speed;
        }
        ...
        if (stateInfo.normalizedTime > 0.9f)
        {
            if (flag == false)
            {
                ball.GetComponent<Rigidbody>().AddForce(-3.1f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2.1f * speed, -3.1f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
    ...
    void OnTriggerEnter(Collider hit)
    {
        if (hit.CompareTag("ball"))
        {
            if (flag == false)
            {
                hit.gameObject.GetComponent<Rigidbody>().AddForce(-3.1f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2.1f * speed, -3.1f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
                hit.gameObject.GetComponent<Rigidbody>().AddTorque(0f, angular_y / 3f, 0f, ForceMode.VelocityChange);
                flag = true;
            }
        }
    }
