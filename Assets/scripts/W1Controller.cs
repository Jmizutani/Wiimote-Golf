...
public class W1Controller : MonoBehaviour {
	private float theta;
	...
	void Start(){
        ...
        golfer = GameObject.Find("golfer_W1");
	...
	void Update(){
        ...
        if (stateInfo.normalizedTime > 0.9f)
        {
            if (flag == false)
            {
                ball.GetComponent<Rigidbody>().AddForce(-4.8f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2.4f * speed, -4.8f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
	...
	void OnTriggerEnter(Collider hit) {
		if (hit.CompareTag("ball")) {
            if (flag == false)
            {
                hit.gameObject.GetComponent<Rigidbody>().AddForce(-4.8f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2.4f * speed, -4.8f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
                hit.gameObject.GetComponent<Rigidbody>().AddTorque(0f, angle_roll, 0f, ForceMode.VelocityChange);
                flag = true;
            }
        }
	}
