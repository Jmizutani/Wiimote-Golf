...
public class W4Controller : MonoBehaviour {
    private float theta;
    ...
    void Start()
    {
        ...
        golfer = GameObject.Find("golfer_W4");
    ...
    void Update()
    {
        ...
        if (stateInfo.normalizedTime > 0.9f)
        {
            if (flag == false)
            {
                ball.GetComponent<Rigidbody>().AddForce(-4.6f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2.3f * speed, -4.6f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
    ...
    void OnTriggerEnter(Collider hit)
    {
        if (hit.CompareTag("ball"))
        {
            if (flag == false)
            {
                hit.gameObject.GetComponent<Rigidbody>().AddForce(-4.6f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2.3f * speed, -4.6f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
