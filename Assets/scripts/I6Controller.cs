...
public class I6Controller : MonoBehaviour
{
    private float theta;
    ...
    void Start()
    {
        ...
        golfer = GameObject.Find("golfer_I6");
    ...
    void Update()
    {
        ...
        if (stateInfo.normalizedTime > 0.9f)
        {
            if (flag == false)
            {
                ball.GetComponent<Rigidbody>().AddForce(-3.3f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2.1f * speed, -3.3f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
    ...
    void OnTriggerEnter(Collider hit)
    {
        if (hit.CompareTag("ball"))
        {
            if (flag == false)
            {
                hit.gameObject.GetComponent<Rigidbody>().AddForce(-3.3f * speed * Mathf.Sin(Mathf.Deg2Rad * (theta + alpha)), 2.1f * speed, -3.3f * speed * Mathf.Cos(Mathf.Deg2Rad * (theta + alpha)), ForceMode.VelocityChange);
